...
from hris.utils.utils import check_day_status
...
from hris.utils.utils import get_dates_diff
...
class CustomLeaveApplicaion(Document):
	def get_feed(self):
		return _("{0}: From {0} of type {1}").format(self.employee_name, self.leave_type)

	def before_insert(self):
		# self.check_leave_dates()
		pass
	...
	def check_leave_dates(self):
		...
		while from_date <= to_date:
			check_day_status(self.employee, from_date, False)
			from_date = add_days(from_date, 1)

	def validate(self):
		status = frappe.db.get_value("Employee", self.employee, "status")
		if status != "Active":
			frappe.throw(_(frappe.bold("Employee Is Not Active")))
		validate_active_employee(self.employee)
		set_employee_name(self)
		self.validate_dates()
		self.check_relieving_date()
		...
		self.validate_advance_leave()

	def validate_advance_leave(self):
		if self.leave_type == "Annual Leave" and cint(self.total_leave_days) < 15:
			self.request_advance_leave = 0
	...
	def validate_applicable_after(self):
		if self.leave_type:
			...
			if leave_type.applicable_after > 0:
				...
				number_of_days = date_diff(
					getdate(self.from_date), date_of_joining)
				if number_of_days >= 0:
					holidays = 0
	...
	def get_allocation_based_on_application_dates(self) -> Tuple[Dict, Dict]:
		...
		allocation_based_on_from_date = _get_leave_allocation_record(
			self.from_date)
		allocation_based_on_to_date = _get_leave_allocation_record(
			self.to_date)
	...
	def update_attendance(self):
		...
		for dt in daterange(getdate(self.from_date), getdate(self.to_date)):
			...
			attendance_name = frappe.db.exists(
				"Attendance", dict(employee=self.employee,
	...
	def validate_salary_processed_days(self):
		...
		# if getdate(self.from_date).day > cint(hrms_settings.deductions_start_day):
		#     startdate = self.from_date
		# else:
		#     startdate = add_months(self.from_date,-1)
		#
		# start_date = getdate(str(getdate(startdate).year)+"-"+str(getdate(startdate).month)+"-"+ str(hrms_settings.deductions_start_day))
		#
		# enddate = add_months(startdate,1)
		# end_date = str(getdate(enddate).year)+"-"+str(getdate(enddate).month)+"-"+str(hrms_settings.deductions_end_day)
		...
		last_processed_pay_slip = frappe.db.sql(
			...
			WHERE docstatus = 1 
				...
				AND (%s <= DATE_SUB(start_date, INTERVAL 15 DAY) AND %s <= DATE_ADD(end_date, INTERVAL 15 DAY))
		...
		#     """
		# 	select start_date, end_date from `tabSalary Slip`
		# 	where docstatus = 1 and  payroll_frequency = 'Monthly'  and employee = %s
		# 	and ((%s between start_date and end_date) or (%s between start_date and end_date))
		# 	order by modified desc limit 1
		# """,
		#     (self.employee,add_days(self.to_date, -15), add_days(self.from_date, -15)),
		#     # (self.employee, start_date, end_date),
		#     debug=True
		# )
		...
		#         ).format(
		#             formatdate(last_processed_pay_slip[0][0]), formatdate(
		#                 last_processed_pay_slip[0][1])
		#         )
		#     )
	...
	def validate_balance_leaves(self):
		if self.from_date and self.to_date:
			...
			if not is_lwp(self.leave_type):
				...
				leave_balance_for_consumption = leave_balance.get(
					"leave_balance_for_consumption")

				if getdate(self.to_date).month > getdate().month or getdate(self.to_date).year > getdate().year:
					yearly_vacation = frappe.db.get_value(
						"Employee", self.employee, "yearly_vacation")
					if not yearly_vacation:
						frappe.throw(
							_("Please set Employee Leave Category in Employee."))

					days_in_month = get_last_day(getdate())
					period_details = get_dates_diff(
						days_in_month, self.to_date)
					period_for_credit = flt(
						period_details["months"]) + flt(flt(period_details["days"])/30)
					additional_credit = (
						flt(yearly_vacation)/12) * period_for_credit
					if additional_credit < 0:
						additional_credit = 0
					leave_balance_for_consumption = round(
						(additional_credit+leave_balance_for_consumption), 2)
					# self.leave_balance_to_the_to_date = leave_balance_for_consumption
				# else:
				# if self.leave_type!="Annual Leave":
				if not self.leave_balance_to_the_to_date:
					self.leave_balance_to_the_to_date = self.total_leave_days

				if self.status != "Rejected" and (flt(self.leave_balance_to_the_to_date) < flt(self.total_leave_days) or not flt(self.leave_balance_to_the_to_date)):
					if frappe.session.user == "mohamed.ismail@ptco.com.sa":
						frappe.msgprint("leave_balance_to_the_to_date===="+str(
							self.leave_balance_to_the_to_date)+"======total_leave_days==="+str(self.total_leave_days))
	...
	def check_futural_credit(self):
		...
		allocation_records = get_leave_allocation_records(
			self.employee, self.posting_date, self.leave_type)
		allocation = allocation_records.get(self.leave_type, frappe._dict())
		end_date = allocation.to_date if cint(
			consider_all_leaves_in_the_allocation_period) else self.posting_date
		cf_expiry = get_allocation_expiry_for_cf_leaves(
			self.employee, self.leave_type, self.to_date, self.posting_date)

		leaves_taken = get_leaves_for_period(
			self.employee, self.leave_type, allocation.from_date, end_date)
		remaining_leaves = get_remaining_leaves(
			allocation, leaves_taken, self.posting_date, cf_expiry)
		...
		if self.leave_type != "Annual Leave":
			self.leave_balance_to_the_to_date = remaining_leaves["leave_balance"]
		else:
		...
		from hrms.hrms.doctype.end_of_service.end_of_service import get_Leave_balance_adjustment_days
		balance_adjustment_days = get_Leave_balance_adjustment_days(
			self.employee, self.to_date) or 0
		leave_balance_to_the_to_date += balance_adjustment_days
		return leave_balance_to_the_to_date

	def check_futural_credits(self):
		if not is_lwp(self.leave_type):
			leave_balance = get_leave_balance_on(
				self.employee,
				self.leave_type,
		...
		leave_balance_to_the_to_date = 0
		if getdate(self.from_date).month > getdate().month or getdate(self.from_date).year > getdate().year:
			self.leave_balance_to_the_to_date = 0
			yearly_vacation = frappe.db.get_value(
				"Employee", self.employee, "yearly_vacation")
			if not yearly_vacation:
				frappe.throw(
			...
			days_in_month = get_last_day(getdate())
			period_details = get_dates_diff(days_in_month, self.from_date)
			period_for_credit = flt(
				period_details["months"]) + flt(flt(period_details["days"]) / 30)
			additional_credit = (flt(yearly_vacation) / 12) * period_for_credit
			leave_balance_for_consumption = round(
				(flt(additional_credit) + (flt(self.leave_balance) if flt(self.leave_balance) else 0)), 2)

			self.leave_balance_to_the_to_date = leave_balance_for_consumption
		else:
			self.leave_balance_to_the_to_date = self.leave_balance

		leave_balance_to_the_to_date = self.leave_balance_to_the_to_date
		return leave_balance_to_the_to_date

	@frappe.whitelist()
	def check_relieving_date(self):
		relieving_date = frappe.db.get_value(
			"Employee", self.employee, "relieving_date")
		if relieving_date and self.from_date and getdate(self.from_date) > getdate(relieving_date):
			msg = frappe.bold("From Date Should be Less than Relieving Date")
			frappe.throw(_(msg))

		if relieving_date and self.to_date and getdate(self.to_date) > getdate(relieving_date):
			msg = frappe.bold("To Date Should be Less than Relieving Date")
			frappe.throw(_(msg))
	...
	def validate_leave_overlap(self):
		...
		for d in frappe.db.sql(
			...
			where employee = %(employee)s and docstatus < 2 and status in ("Open", "Approved","requested")
	...
	def throw_overlap_error(self, d):
		...
		msg = _("Employee {0} has already applied for {1} between {2} and {3} : {4}").format(
			self.employee, d["leave_type"], formatdate(
				d["from_date"]), formatdate(d["to_date"]), form_link
		)
	...
	def get_total_leaves_on_half_day(self):
		leave_count_on_half_day_date = frappe.db.sql(
			...
			and name != %(name)s""",
			{"employee": self.employee,
			 "half_day_date": self.half_day_date, "name": self.name},
		)[0][0]
	...
	def validate_max_days(self):
		...
		if max_days and self.total_leave_days > cint(max_days):
			frappe.throw(_("Leave of type {0} cannot be longer than {1}").format(
	...
	def validate_optional_leave(self):
		...
		while day <= getdate(self.to_date):
			if not frappe.db.exists(
					{"doctype": "Holiday", "parent": optional_holiday_list,
						"holiday_date": day}
			):
				frappe.throw(
					_("{0} is not in Optional Holiday List").format(
						formatdate(day)), NotAnOptionalHoliday
				)
			day = add_days(day, 1)
	...
	def create_separate_ledger_entries(self, alloc_on_from_date, alloc_on_to_date, submit, lwp):
		...
		if leaves_in_first_alloc:
			args.update(
				dict(from_date=self.from_date, to_date=first_alloc_end,
		...
		if leaves_in_second_alloc:
			args.update(
				dict(from_date=second_alloc_start, to_date=self.to_date,
	...
	def create_ledger_entry_for_intermediate_allocation_expiry(self, expiry_date, submit, lwp):
		...
		if getdate(expiry_date) != getdate(self.to_date):
			...
			if leaves:
				args.update(dict(from_date=start_date,
							to_date=self.to_date, leaves=leaves * -1))
				create_leave_ledger_entry(self, args, submit)
...
def get_leave_details(employee, date):
	...
	for d in allocation_records:
		...
		# check if any leave adjustment balance found
		leave_adjustment_balance = get_leave_adjustment_balance(
			employee, d, date)

		allocation.total_leaves_allocated += leave_adjustment_balance

		remaining_leaves = get_leave_balance_on(
			employee, d, date, to_date=allocation.to_date, consider_all_leaves_in_the_allocation_period=True
		)
		remaining_leaves += leave_adjustment_balance
...
@frappe.whitelist()
def get_leave_balance_on_with_leave_adjustment(
	employee: str,
	leave_type: str,
	date: str,
	to_date: str = None,
	...
	if for_consumption:
		return remaining_leaves + leave_adjustment_balance
	else:
		return remaining_leaves.get("leave_balance") + leave_adjustment_balance



def get_leave_adjustment_balance(employee_code, leave_type, date):
	""" 
	if adjustment_transaction_date >= date : add adjustment_days to total leaves allocated 
	"""
	total_adjustment_balance_days = 0
	LA_from_date, LA_to_date = get_leave_allocation_dates(
		employee_code, leave_type, date)
	if LA_from_date and LA_to_date:
		leave_adjustment_balance_days = frappe.db.get_all("Leave Balance Adjustment",
														  {
															  "docstatus": 1,
															  "employee_code": employee_code,
															  "leave_type": leave_type,
															  "adjustment_transaction_date": ['between', [LA_from_date, LA_to_date]],
														  },
...
def get_leave_allocation_dates(employee_code, leave_type, date):
...
def get_leave_balance_on(
	...
	allocation = allocation_records.get(leave_type, frappe._dict())
	end_date = allocation.to_date if cint(
		consider_all_leaves_in_the_allocation_period) else date
	cf_expiry = get_allocation_expiry_for_cf_leaves(
		employee, leave_type, to_date, date)

	leaves_taken = get_leaves_for_period(
		employee, leave_type, allocation.from_date, end_date)
	remaining_leaves = get_remaining_leaves(
		allocation, leaves_taken, date, cf_expiry)
	if for_consumption:
...
def get_remaining_leaves(
	...
	if cf_expiry and allocation.unused_leaves:
		...
		leave_balance_for_consumption = flt(
			allocation.new_leaves_allocated) + flt(remaining_cf_leaves)
...
def add_department_leaves(events, start, end, employee, company):
	...
	filter_conditions = ' and employee in ("%s")' % '", "'.join(
