...
class CustomPayrollEntry(Document):
    ...
    def get_employees_with_unmarked_attendance(self) -> list[dict] | None:
        ...
        for emp in self.employees:
            ...
            start_date, end_date = self.get_payroll_cutt_off_dates_for_employee(details)
    ...
    def get_payroll_cutt_off_dates_for_employee(self, employee_details: dict) -> tuple[str, str]:
        hrms_settings = frappe.db.get_singles_dict("HRMS Settings")
        if not hrms_settings.salary_slip_attendance_start_date:
            frappe.throw(_("Please Set Salary Slip Attendance Start Date in HRMS Settings"))

        startdate = add_months(self.start_date, -1)
        start_date = getdate(str(getdate(startdate).year)+"-"+str(getdate(startdate).month)+"-" + str(hrms_settings.salary_slip_attendance_start_date))
        if employee_details.date_of_joining > getdate(start_date):
            start_date = employee_details.date_of_joining

        end_date = getdate(str(getdate(self.end_date).year)+"-"+str(getdate(self.end_date).month)+"-" + str(hrms_settings.salary_slip_attendance_end_date))
        if employee_details.relieving_date and employee_details.relieving_date < getdate(end_date):
    ...
    def get_employee_and_attendance_details(self) -> list[dict]:
        ...
        if not hrms_settings.salary_slip_attendance_start_date:
            frappe.throw(_("Please Set Salary Slip Attendance Start Date in HRMS Settings"))
        startdate = add_months(self.start_date, -1)
        start_date = getdate(str(getdate(startdate).year) + "-" + str(getdate(startdate).month) + "-" + str(
            hrms_settings.salary_slip_attendance_start_date))
        end_date = getdate(str(getdate(self.end_date).year) + "-" + str(getdate(self.end_date).month) + "-" + str(
            hrms_settings.salary_slip_attendance_end_date))
